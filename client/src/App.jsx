...
function App() {
  ...
  useEffect(() => {
    if (!selectedAI || !selectedTable) return;
    fetch(`${API_URL}/data?model=${selectedAI.name}`)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    if (!selectedAI || !dates) return;
    fetch(`${API_URL}/processData?model=${selectedAI.name}&dates=${dates}`)
      .then(res => res.json())
      .then(processData => {
        console.log(dates);
      ...
      .catch(err => console.error("Error fetching Processing Data:", err));
  }, [selectedAI, dates]);
